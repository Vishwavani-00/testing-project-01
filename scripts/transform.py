...
Cleans stg_superstore and loads star schema tables:
...
def load_dim_date(conn, df: pd.DataFrame):
    dates = pd.concat([
    ...
    conn.commit()
    logger.info(f"dim_date: {len(dates)} rows upserted.")


def load_dim_customer(conn, df: pd.DataFrame):
    customers = df[["customer_id", "customer_name", "region"]].drop_duplicates(subset=["customer_id"])
    with conn.cursor() as cur:
        for _, row in customers.iterrows():
            ...
            """, (row["customer_id"], row.get("customer_name"), row["region"]))
    conn.commit()
    logger.info(f"dim_customer: {len(customers)} rows upserted.")
...
def load_dim_product(conn, df: pd.DataFrame):
    ...
    with conn.cursor() as cur:
        for _, row in products.iterrows():
            ...
            """, (row["product_id"], row.get("product_name"), row.get("category"), row.get("sub_category")))
    conn.commit()
    logger.info(f"dim_product: {len(products)} rows upserted.")
...
def load_dim_region(conn, df: pd.DataFrame):
    ...
    with conn.cursor() as cur:
        for region in regions:
            cur.execute("INSERT INTO dim_region (region_name) VALUES (%s) ON CONFLICT (region_name) DO NOTHING;", (region,))
    conn.commit()
    logger.info(f"dim_region: {len(regions)} regions upserted.")
...
def load_fact_sales(conn, df: pd.DataFrame):
    ...
    df_fact = df[[c for c in fact_cols if c in df.columns]].copy()
    df_fact["order_date"] = df_fact["order_date"].dt.date
    if "ship_date" in df_fact.columns:
        df_fact["ship_date"] = pd.to_datetime(df_fact["ship_date"], errors="coerce").dt.date
    ...
    with conn.cursor() as cur:
        for _, row in df_fact.iterrows():
            cur.execute("""
                ...
                SET sales=EXCLUDED.sales, profit=EXCLUDED.profit, discount=EXCLUDED.discount,
                    profit_margin=EXCLUDED.profit_margin, loaded_at=NOW();
            """, (
                row["order_id"], row["customer_id"], row["product_id"],
                row["order_date"], row.get("ship_date"), row["region"],
                float(row["sales"]), int(row["quantity"]), float(row["discount"]),
                float(row["profit"]), float(row["profit_margin"]),
                int(row["order_month"]), int(row["order_year"])
            ))
    conn.commit()
    logger.info(f"fact_sales: {len(df_fact)} rows upserted.")
